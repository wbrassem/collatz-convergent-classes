...
\title{On the Natural Density of Convergent Cylinders in the Collatz Map}
...
We encode trajectories of $f$ by finite parity words (equivalently, sequences of
2-adic division counts), each of which induces an affine transformation describing
the action of $f$ on entire congruence classes simultaneously. These words generate
a hierarchical decomposition of the positive integers into residue classes, which
we identify canonically with a family of nested dynamical cylinders.

Within this framework, minimal stopping-time classes correspond to a disjoint
partition of the integers into cylinders indexed by admissible parity words. We
derive a symbolic grammar governing admissible word growth and use it to control
the combinatorial expansion of these classes.

Comparing this growth with the exponential scaling of cylinder measures $2^{-k}$,
we show that the total natural density of all convergent cylinders is one. Equivalently,
the complement of their union has natural density zero. Thus almost every positive
integer eventually attains a strictly smaller value under iteration of $g$ (in the
sense of natural density).

While zero-density exceptional trajectories are not excluded, the result provides a
measure-theoretic description of typical Collatz dynamics via a canonical cylinder
decomposition of the integer lattice.
...
Uniformity across the class follows from the fact that all elements share the same parity sequence;
this will be formalized and proved in Section~\ref{sec:grammar}.
\end{define}
...
Let a residue class be determined by a parity word of length $m > 0$ with division counts
$(d_0,\dots,d_{m-1})$.  
Then for every integer in the class, the division counts $d_i$ are identical for
all $0 \le i < m$.
\end{lemma}

\begin{remark}
The parity word determining the residue class is understood to terminate at length $m$.
No claim is made about parity symbols or division counts beyond this prefix, which lie
outside the equivalence relation defining the class.
\end{remark}
...
\]
However, the symbolic composition of the accelerated map is unique: at each stage,
the exponent \(d_i\) is exactly \(v_2(3 f^i(x)+1)\), so the sequence of exponents is
uniquely determined by the successive 2-adic valuations along the trajectory,
and hence by the parity word itself. Therefore \(\omega = \omega'\), a contradiction.

Consequently, distinct parity words determine distinct and disjoint residue classes.
\end{proof}
...
Thus admissible parity words are in one-to-one correspondence with the residue
classes they induce.
\end{proof}
...
Consider a row produced by a \emph{single} increment step with strictly positive entries 
and total sum $S$ (i.e., the sum of its entries in the horizontal-sum table). 
If a \emph{double} increment is applied immediately afterward, producing a row whose 
terminal entry equals the sum of the previous row and all other entries remain strictly positive, 
then the total sum of the resulting row is strictly less than $3S$.
\end{lemma}
...
positive linear dynamical system.  The purpose is to show that the dynamics
is order-preserving on a positive cone, so that rows realizing extremal
local growth at any given stage continue to dominate all other admissible
rows under every future evolution.  Consequently, the extremal local growth
calculations of Appendix~\ref{app:local-growth-details} provide \emph{uniform
bounds} valid for all admissible rows, not merely for illustrative examples.
...
\begin{remark}[Reduction to the simplex]
Because both the numerator and denominator in the definition of $\alpha(P)$ are
positive linear functionals, the ratio
\[
\frac{\sum_i (T_P v)_i}{\sum_i v_i}
\]
is invariant under positive scaling of $v$.  Hence the supremum may be taken over
the normalized simplex
\[
\Delta = \left\{ v \in \mathbb{R}^{k_0}_{\ge 0} : \sum_i v_i = 1 \right\},
\]
where $k_0$ is the length of the input row. On $\Delta$, the ratio reduces to a linear functional:
\[
\sum_i (T_P v)_i
= \sum_j \left( \sum_i (T_P)_{ij} \right) v_j.
\]
Since $\Delta$ is a compact convex polytope, the maximum of this linear functional
is attained at an extreme point of $\Delta$, which corresponds to a coordinate
vector concentrating all mass in a single coordinate $j$.

Thus the supremum defining $\alpha(P)$ is realized by rows supported on coordinates
$j$ for which the column sum $\sum_i (T_P)_{ij}$ is maximal.  These rows are exactly
those that saturate the growth inequalities of
Lemma~\ref{lem:single-double-bound}.
\end{remark}
...
increment word (whose total length is $k$) gives
\[
\frac{S_{n+k}}{S_n}
\le
...
The OEIS sequences \OEIS{A020914} and \OEIS{A186009} employ differing indexing
...
While the first doubling increment in \OEIS{A020914} occurs at index $1$, its
