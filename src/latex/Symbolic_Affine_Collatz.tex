...
An odd integer $x \in \mathbb{N}$ realizes the division word $\omega$ if
\[
...
Each (not necessarily realizable) division word \(\omega\) of length \(m\)
determines a formal affine map
\[
F_\omega(x)=\frac{3^m x+c(\omega)}{2^{D(\omega)}},
\]
...
\begin{remark}
The quantity \(D(\omega)\) records the total exponent of 2 removed along
the trajectory segment encoded by \(\omega\). It determines the modulus
\(2^{D(\omega)}\) appearing in the associated affine representation above.

A division word is called \emph{realizable} if it is realized by at least one
odd integer. In later sections we use the synonymous term
\emph{admissible} when emphasizing the arithmetic structure of realizable
division words.
\end{remark}
...
rule. Therefore this map is precisely the affine representation associated
with the concatenated word
$\omega\eta$.
...
\begin{remark}
Contractivity refers only to the finite trajectory encoded by
\(\omega\). Thus
\[
F_\omega(x)=f^m(x)<x,
\]
but no conclusion is drawn about the behavior of later iterates or
eventual convergence to \(1\).
\end{remark}
...
contraction \(2^{D(\omega)}\). For a division word \(\omega\) of length
\(m\), the associated affine map has linear coefficient
\[
\frac{3^m}{2^{D(\omega)}}.
\]
...
\begin{itemize}
    \item if \(2^{D(\omega)} < 3^m\): expansive slope.
    \item if \(2^{D(\omega)} > 3^m\): contractive slope.
\end{itemize}
...
\]
which is the OEIS sequence \OEIS{A020914}.  This is the minimal integer \(k\) satisfying
\[
2^k>3^m.
\]

Consequently, \(B(m)=A020914(m)\) represents the minimal total division count
...
x \equiv 15 \pmod{128}.
\]
This congruence determines a unique residue class modulo $2^7$, and
defines the arithmetic constraint associated with the division word.
In Section~\ref{sec:cylinder-geometry}, these congruence classes will
be identified with the cylinder representation of admissible words.
...
The word $\omega = (1,1,1,4)$ records three successive divisions by 2,
followed by a division by \(2^4\). The affine representation packages
this symbolic information into a single arithmetic transformation together
with its associated congruence condition.

Thus $d_3=4$ means that the fourth iterate removes exactly four factors of \(2\).
...
\noindent
The affine representation is purely symbolic: it associates an affine
transformation with every formal division word. The following sections
determine which formal words are realizable under the accelerated Collatz
map by developing the admissible division grammar and proving its
equivalence to realizability.
