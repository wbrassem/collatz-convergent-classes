...
B(i) := A020914(i),
\]
so that (up to the standard OEIS index shift) the density contribution of
convergent residue classes of stopping time $i$ is
\[
A186009(i+1)\, 2^{-B(i)}.
\]
...
\begin{remark}[Indexing and Effective Delay]
...
two growth steps relative to the denominator.

This distinction affects only constant factors in subsequent bounds and does
not alter the asymptotic dominance of the denominator.
...
$A020914$ and $A186009$ are absorbed into this parameter.

In particular, this effective two-step delay is the only delay relevant
for asymptotic growth comparisons; the remaining offset is purely notational.
...
\[
\frac{A186009(i+1)}{3^i}
\;<\;
A186009(i+1)\,2^{-A020914(i)}
\;<\;
\frac{A186009(i+1)}{2 \cdot 3^i}.
\]
\end{lemma}

\begin{proof}
By definition, $A020914(i)$ is the minimal integer $m$ such that $2^m > 3^i$. Consecutive powers of two differ by a factor of $2$, so
\[
3^i < 2^{A020914(i)} < 2 \cdot 3^i.
\]
Taking reciprocals and multiplying by $A186009(i+1)$ gives the stated bounds.
\end{proof}
...
Let $a_i := A186009(i)$ and $b_i := A020914(i)$.  
...
on the numerator $a_i$ is realized only after a fixed delay
and contributes at most a bounded additive increase per event.

Consequently, for all sufficiently large $i$, the cumulative multiplicative
growth of $2^{b_i}$ strictly dominates that of $a_i$.  In particular,
$a_i = O(2^{D_i})$ while $2^{b_i} = \Theta(2^{i + D_i})$.
\end{lemma}

\subsection{Delayed Doubling and Upper Bound on Numerator Growth}

We now formalize the structural reason why the density contribution of convergent residue classes vanishes asymptotically.

\begin{lemma}[Delayed Prefix Product Bound for $A186009$]
\label{lem:prefix-doubling}
Let $S_n := A186009(n)$ denote the $n$-th term in the horizontal sum construction, and let
$D_n$ denote the cumulative number of ``double'' events up to row $n$. Then there exists a constant $C > 0$ (accounting for initial additive contributions) such that
\[
S_n \;\le\; C \cdot 2^{D_n}.
\]

Consequently, the ratio of numerator to denominator satisfies
\[
\frac{S_n}{2^{A020914(n)}} \;\le\; \frac{C \cdot 2^{D_n}}{2^{n + D_n}} = \frac{C}{2^n} \;\longrightarrow\; 0 \quad \text{as } n \to \infty.
\]
\end{lemma}

\begin{proof}
By the construction of $A186009$, each row $S_n$ is generated from the previous row $S_{n-1}$ via a sum of all but the last term, plus a final term which is either unchanged (single) or doubled (double). Denote the number of terms in row $n$ by $T_n = D_n + 1$.  

\begin{itemize}
    \item For a single (after delay), the additive contribution is at most $S_{n-1}$.  
    \item For a double (after delay), the additive contribution is at most $2 \cdot S_{n-1}$.  
\end{itemize}

Applying this recursively over all $D_n$ doubles, and absorbing the initial terms in a constant $C$, yields the bound
\[
S_n \le C \cdot 2^{D_n}.
\]

Meanwhile, the denominator $2^{A020914(n)}$ grows as the smallest power of 2
exceeding $3^n$, which can be expressed as $2^{n + D_n}$, since each double in
the numerator corresponds to an immediate power of 2 in the denominator. Indeed,
each step contributes at least one power of two, with each double event contributing
exactly one additional power.  Hence
\[
\frac{S_n}{2^{A020914(n)}} \le \frac{C \cdot 2^{D_n}}{2^{n + D_n}} = \frac{C}{2^n} \to 0 \quad \text{as } n \to \infty,
\]
demonstrating the asymptotic vanishing of the density contribution.
\end{proof}

\subsubsection*{Concrete Example}

Consider $A186009(13) = (1, 10, 52, 182, 485, 961, 961) = 2652$, a double. The next row, $A186009(14) = (1, 11, 63, 245, 730, 1691, 2652, 2652) = 8045$, is also a double.  

\begin{itemize}
    \item Counting doubles: $D_{13} = 6$, $D_{14} = 7$, giving $2^{D_{14}} = 128$.  
    \item Applying the lemma: $S_{14} \le C \cdot 2^7 = 128 \cdot C$, while the denominator grows like $2^{14 + D_{14}} = 2^{21}$, so
    \[
    \frac{S_{14}}{2^{A020914(14)}} \le \frac{C}{2^{14}} \ll 1.
    \]
\end{itemize}

This illustrates how the delayed realization of doubling events in the
numerator is structurally outpaced by their immediate realization in the
denominator, ensuring that each successive density contribution
\[
d_i := A186009(i)\,2^{-A020914(i)}
\]
associated with stopping time $i$ vanishes as $i \to \infty$.

\subsubsection*{Conclusion}

The growth of both the numerator \(A186009(i)\) and the denominator
\(2^{A020914(i)}\) is governed by the same underlying increment sequence
...
by a fixed offset of $2$ arising from the horizontal-sum (parity-word)
construction of \(A186009\).

As a result, the denominator consistently maintains a \emph{phase advantage}
over the numerator: each multiplicative growth event appears earlier in
\(2^{A020914(i)}\) than in \(A186009(i)\).  This phase advantage is uniform
