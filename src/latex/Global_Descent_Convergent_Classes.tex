...
the expansion is always at index 1 and thus the formula henceforth (producing the same
expansion components) is modified as follows.
...
last term remains the same – namely that it occurs when $A022921(n-2) =  2$, which is precisely
the single/double grammar governing admissible growth in the preceding appendices, and hence the
local multiplicative structure used in the extremal bounds.

Prepending 1 to \OEIS{A100982} produces the \OEIS{A186009} series.  The doubling formula changes
slightly as a result of the new leading term and occurs when $A022921(n-3) =  2$ due to indexing
conventions of \OEIS{A186009} compared with \OEIS{A022921}.
