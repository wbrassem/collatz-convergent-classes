...
lexicographically minimal admissible increment pattern.
\end{lemma}
...
baseline. Consequently, the admissible local patterns in the
\(\{s,d\}\)-grammar determine the combinatorial growth of
...
Let a row generated by a single step have total sum $k$, and let its final
(rightmost) entry also equal $k$. Then the row obtained by applying a double
step immediately afterward has total sum at most $3k$.
\end{lemma}

\begin{proof}
Under a single step, row entries are generated by Pascal-type summation and
are strictly increasing left to right. In this case, the sum of all entries
preceding the final entry is at most the final entry itself. Thus, if the
final entry equals $k$, the total row sum satisfies
\[
\sum_{\text{preceding entries}} \le k.
\]

A double step duplicates the final entry and appends it as a new terminal
entry, without altering the preceding entries. Consequently, the resulting
...
Every admissible infinite sequence of increments decomposes uniquely into concatenations
...
is pointwise dominated by sequences composed of repeated 41-cycles or by
longer blocks containing strictly more contractive content per symbol. 
...
so every positive integer belongs to some convergent residue class of finite
stopping time.
...
From Global Descent it follows immediately that no positive integer can remain
perpetually above its initial value, leaving only the trivial cycle $1\to2\to4$
as a candidate for a recurrent orbit.
...
The maximal geometric growth rate achievable by any admissible infinite word
is bounded by
\[
\rho_{41}
=
...
The constants appearing in Lemma~\ref{lem:local-growth} are obtained by a
complete extremal analysis of the horizontal-sum dynamics governing
...
each with a distinct instantaneous growth behavior:
\[
\begin{array}{c|c}
...
of the same underlying recurrence.  Since the horizontal--sum dynamics and the
single--to--double bound of Lemma~\ref{lem:single-double-bound} apply uniformly
to all admissible rows initiated at a single step, any upper bound obtained
from a concrete instance constitutes a valid global upper bound for the
corresponding local pattern.
