...
The admissible division words form a forest of rooted prefix trees
(Section~\ref{sec:cylinders-structure}), indexed by admissible
cylinders. Each node corresponds to a unique admissible division
word, while depth corresponds to word length.

We now study the combinatorial growth of this admissible forest.
Let
\[
N(m)
\]
denote the total number of admissible division words of length \(m\),
equivalently the total number of nodes at depth \(m\) across all
trees in the forest. By construction,
\[
N(m)=A186009(m+1).
\]
...
Admissible words concatenate only when their associated cylinders are compatible. More 
precisely, if $\omega$ and $\eta$ are admissible words of lengths $m$ and $n$, then any
admissible extension corresponding to a word of length $m+n$ must lie in
\[
C_\omega \cap f^{-m}(C_\eta),
\]
where $f^{-m}$ denotes the backward symbolic shift induced by the valuation dynamics.
By the prefix nesting property of cylinders, this intersection is either empty or a
single refined cylinder determined by a unique admissible extension of $\omega$.  Thus
concatenation produces at most finitely many admissible extensions, with uniform branching
control independent of depth.
...
\begin{proof}
Each admissible word of length $m+n$ determines uniquely:
\begin{itemize}
\item a prefix of length $m$, and
\item a compatible extension of length $n$.
\end{itemize}

By the cylinder structure, every prefix corresponds to a residue class modulo
$2^{D(\omega)}$, and extensions are determined by refinement of this class under
additional Beatty constraints.

Since each cylinder splits into only finitely many admissible refinements
(controlled by the jump/propagation structure of $B(n)$), the number of extensions
per prefix is uniformly bounded.

Thus the number of length-$m+n$ words is at most a constant times the product
of counts at levels $m$ and $n$.
\end{proof}
...
\begin{proof}
Follows immediately from submultiplicativity via Fekete-type arguments.
\end{proof}
